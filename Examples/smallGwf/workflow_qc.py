...
def qc(data_folder):
    inputs  =  [f"{data_folder}/part{number:>03}.fq" for number in range(1, parts + 1)]
    outputs = ['reports/multiqc_report.html']
    options = {"cores": 1, "memory": "1g", "walltime": "02:00:00"}
    spec = f"""
    mkdir -p reports
...
# 4. QC reports
gwf.target_from_template( "qc", qc(data_folder = "gwf_splitted") )