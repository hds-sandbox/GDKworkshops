...
- We have hands-on as well, **follow the slides** from [tinyurl.com/pipelinesGDK](https://tinyurl.com/pipelinesGDK)
...
conda create -y -n pipelineEnv gwf
...
Other useful tips when developing a `gwf` workflow:

- you can write your templates library in a separate file (e.g. `templates.py`) and import them in your workflow file (e.g. `workflow.py`), to keep it more organized
- your workflow is a python script! Use all the advantages of it, for example:
  - use loops to create targets from templates, instead of writing each target separately
  - find files or parse their names with python functions
  - test a step of the workflow directly in python instead of running it as a script
- generalize workflows through templates

---

:::{.callout-note title="Take home messages"}

 - A workflow manager interacts with the queueing manager of the cluster and organize dependencies
 - GWF works with `slurm` and other backends, so it is portable locally and on many other clusters
