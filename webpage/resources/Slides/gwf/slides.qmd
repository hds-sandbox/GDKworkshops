...
flowchart LR
  A0(["Start"]) --->|data.txt<br>INPUT| A[Rename<br>TARGET]
  A --->|samples.txt<br>OUTPUT-INPUT| B[Zip<br>TARGET]
...
gantt
    dateFormat  HH:mm
    axisFormat  %H:%M
    title HPC Queue Durations vs. Resources
...
A lightweight and easy to adopt workflow manager. It requires only some basic `Python` - you can learn it along the way starting from examples. Some useful features:
...
:::{.callout-tip}
You do not need the option `-f workflow.py` if your workflow file has the name `workflow.py`, which is the default for `gwf`.
:::
...
```{.python .scrollable  code-line-numbers="4-6|9|12-21|26-35"}
{
    "split": {
        "options": {
            "cores": 1,
...
You need to:
  ...
  - create a new conda environment called `qcEnv` where you install the two packages `fastqc multiqc`.
