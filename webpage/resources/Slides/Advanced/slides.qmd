...
- `--no-home`: Prevents the container from automatically binding the user's home directory. Useful when you want to avoid exposing your home directory to the container.

- `--cleanenv`: Clears all environment variables except those explicitly set for the container. This ensures a clean environment for the container to run.

- `--nv`: Enables NVIDIA GPU support by binding the necessary libraries and drivers into the container. Useful for running GPU-accelerated applications. Equivalent for AMD GPUs is `--rocm` (not the case on GDK).

- `--pwd`: Sets the working directory inside the container. Useful for ensuring the container starts in a specific directory.

- `--app`: Runs a specific application defined in the container's metadata. Useful for containers with multiple applications.
...
:::{.callout-warning .hidden}
This way of using Rstudio is quite complex and has some difficulties in adding packages. Though it is able to run packages in your current conda/mamba/pixi environment, which you can keep up to date with new packages.

The suggested procedure is to use the virtual desktop on your browser, install `conda-forge::rstudio-desktop` in your environment together with the R packages you need, then run the command `rstudio` in an interactive job (maybe in a `tmux` session). That needs no tunnelling and works always out of the box.

See the [Introductory workshop slides](https://hds-sandbox.github.io/GDKworkshops/resources/Slides/Introduction/slides.html#/software-management) section where we have an exercise to run Rstudio on the virtual desktop. 
:::
...
---
 ...
 make a tmux sessions

 tmux new -s pipeline

 wget https://zenodo.org/api/files/436d1854-d638-4fdb-8d6b-7c566ed13af8/bioinformatic_tutorial_files.tar.gz -O data.tar.gz
 tar -xvf data.tar.gz
 rm data.tar.gz
 gunzip bioinformatic_tutorial_files/*.gz
...
conda install -y -c micknudsen gwf-utilization
