...
#index reference
bwa-mem2 index ref.fasta.gz
#align data
bwa-mem2 mem -t 4 ref.fasta.gz \
             data.fastq \
            | samtools sort \
            -@ 3 \
            -n \
...
#align data
pixi run bwa-mem2 mem -t 4 ref.fasta.gz \
             data.fastq \
            | samtools sort \
            -@ 3 \
            -n \
...
```{.bash}
 singularity run samtools_docker.sif \
                 samtools view \
...
If you want to build containers with Docker, you need to 
  ...
  - build locally with Docker or Singularity, and then upload the container to a repository (e.g. DockerHub or Quay). 
  - pull them on GenomeDK with singularity

&nbsp;

Singularity is installed on GenomeDK, but because you need root privileges to build a container on the cluster, you cannot do that.
