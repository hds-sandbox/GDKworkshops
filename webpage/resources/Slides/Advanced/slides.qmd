...
Now you have done all this changes to your `~/.bashrc` file, you need to apply them. You can either logout and login again, or simply run
...
An archive (incremental) copy can be done with the options `a`. You can add a progress bar with `P`. You can exclude files: here, for example, we want only the file with `fastq` extension. Run the command
...
You then open VScode, connect on the node, and use the file browser of VScode to open your notebooks. When the notebook is open, you can also choose all the jupyter kernels related to your conda or pixi environment where you have jupyterlab installed!
